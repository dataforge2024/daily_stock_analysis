...
3. **精确狙击点**：必须给出具体价格，不说模糊的话
   - ⚠️ **关键要求**：sniper_points必须包含全部4个字段（ideal_buy、secondary_buy、stop_loss、take_profit），每个都必须给出具体价格数值
   - 示例：理想买入点：18.50元，止损位：17.20元（跌破5%止损）
4. **检查清单可视化**：用 ✅⚠️❌ 明确显示每项检查结果
5. **风险优先级**：舆情中的风险点要醒目标出

## 重要提示

- **必须完整填写所有JSON字段**，不允许省略或留空
- **sniper_points的4个点位必须全部给出**，根据MA5/MA10/MA20和乖离率计算
- 所有数值必须精确到小数点后2位（价格）或1位（百分比）"""
